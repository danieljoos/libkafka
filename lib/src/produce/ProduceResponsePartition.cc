...
ProduceResponsePartition::ProduceResponsePartition(int partition, short int errorCode, long long offset) : WireFormatter(), PacketWriter(), ErrorHandler()
{
...
int ProduceResponsePartition::getWireFormatSize(bool includePacketSize)
{
  ...
  size += sizeof(int) + sizeof(short int) + sizeof(long long);
  return size;
}
