...
FetchResponsePartition::FetchResponsePartition(Packet *packet) : WireFormatter(), PacketWriter(packet), ErrorHandler()
{
  ...
  // Kafka Protocol: long long highwaterMarkOffset
...
FetchResponsePartition::FetchResponsePartition(int partition, short int errorCode, long long highwaterMarkOffset, int messageSetSize, MessageSet *messageSet, bool releaseArrays) : WireFormatter(), PacketWriter(), ErrorHandler()
{
...
int FetchResponsePartition::getWireFormatSize(bool includePacketSize)
{
  ...
  size += sizeof(int) + sizeof(short int) + sizeof(long long) + sizeof(int) + this->messageSetSize;
  return size;
}
